...
import { quotes, cases, partners, clients } from '../data/references.json'
...
export const renderClientLogos = () => {
  ...
  const singleRowWidth = innerWidth / clients.length;
  const doubleRowWidth = singleRowWidth * 2;
  const width = innerWidth < 768 ? doubleRowWidth : singleRowWidth;

  return clients.map((client) =>
    <div key={ client } className="client-logo py-2 px-3 px-lg-4 d-flex" style={{ width }}>
      <img 
        width="100%"
        alt={ client }
        src={ `assets/svg/logos/${client}_white.svg`}
        className="my-auto" />
    </div>
  );
}
